...
bool AsciiWriter::Write( const wchar_t* text )
{
	bool ret = Write( text, wcslen(text) );
	if(ret==false)
	{
		errNum = ELMAX_WRITE_ERROR;
		err = GetErrorMsg(errNum);
...
bool AsciiWriter::WriteLine( const wchar_t* text )
{
	bool ret = WriteLine( text, wcslen(text) );
	if(ret==false)
	{
		errNum = ELMAX_WRITE_ERROR;
		err = GetErrorMsg(errNum);
