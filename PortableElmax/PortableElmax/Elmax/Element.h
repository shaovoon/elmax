...
{
public:
	Root_RAII(Elmax::Element& root) : m_Root(root) {}
	~Root_RAII() { m_Root.Destroy(); }
private:
	Elmax::Element& m_Root;
};

#ifdef MAKE_WRITER_ROOT
#undef MAKE_WRITER_ROOT
#define MAKE_WRITER_ROOT(root, root_name) \
Elmax::Element root(root_name); \
Elmax::Root_RAII raii_writer_##root(root);
#endif

#ifdef MAKE_READER_ROOT
#undef MAKE_READER_ROOT
#define MAKE_READER_ROOT(root) \
Elmax::Element root; \
Elmax::Root_RAII raii_reader_##root(root);
#endif
